...
**`apps/ui`** adds **[Date Field](/ui/docs/components/date-field)**, a field component that composes **`Popover`**, **`Button`**, and **`Calendar`** into one date picker with per-mode typing: **`mode="single"`** holds a `Date`, **`mode="multiple"`** holds a `Date[]`, and **`mode="range"`** holds a `DateRange`, with `value` / `defaultValue` / `onValueChange` supporting controlled and uncontrolled usage. Close-on-select is deterministic per mode — single closes on every select, range and multiple never auto-close (DayPicker's first range click already sets a same-day range) — and **`closeOnSelect`** overrides any of them. The trigger renders per-mode labels ("Pick a date", "Pick a date range", "Pick dates") with the selected value formatted through date-fns format strings via the **`format`** prop, and its content can be replaced with a function that receives the current value. **`DateFieldCalendar`** wires the popup and calendar to the field — reopening lands on the current selection — and passes through every Calendar prop (`numberOfMonths`, `captionLayout`, `min`/`max`, …). **`useDateField`** is the escape hatch for bespoke popup layouts such as a preset sidebar next to the calendar.

Examples cover a label + field composition, a range with two months, and a multiple mode with a badge trigger. The nine [Date Picker](/ui/docs/components/date-picker) examples were rebuilt on `DateField`.
...
New dependency: **`date-fns`** (Date Field, for the synced package copy) — `@daypicker/react` already ships with Calendar.

### Rating

**`apps/ui`** adds **[Rating](/ui/docs/components/rating)**, a star rating input built on Base UI's `RadioGroup` through the coss radio-group, so the stars behave like real radios — radiogroup semantics, arrow-key navigation with `Home`/`End`, and a form-submittable hidden input when the group is given a `name`. **`value`** / **`defaultValue`** / **`onValueChange`** work with plain numbers, where `undefined` means nothing is rated, and **`count`** (default 5) auto-renders that many stars. **`allowClear`** makes clicking the selected star — or pressing Space on it while focused — clear the rating back to `undefined`, the hover preview fills stars up to the pointer and is suppressed when the group is disabled or read-only, and **`size`** switches between `sm` / `default` / `lg`. **`RatingItem`** composes custom star content when the auto-rendered `StarIcon`s aren't enough — set the value per star, override the `aria-label`, and pass any other Radio prop through.

Four examples cover the basic rating, a controlled rating, sizes, and a rating wired into a `Field`.
...
**`apps/ui`** adds **[Tag Input](/ui/docs/components/tag-input)**, a free-form tag composer: `TagInput` manages a `string[]` (controlled or uncontrolled), `TagInputItem` renders a removable chip in the combobox-chip style, and `TagInputInput` adds tags with Enter (trimmed, duplicate-safe) and removes the last tag with Backspace when empty. Clicking the container's padding focuses the input. Compose the three parts to cap the number of tags, validate, or render chips yourself.
...
**`apps/ui`** adds **[Chart](/ui/docs/components/chart)**, a set of themed chart primitives over Recharts 3, organized under a new **chart** category. **[ChartContainer](/ui/docs/components/chart#chartcontainer)** maps series keys to labels and colors through a config object, injects them as scoped `--color-*` CSS variables so series re-theme with light/dark mode, and neutralizes Recharts' default white strokes for dark backgrounds. **[ChartTooltipContent](/ui/docs/components/chart#charttooltipcontent)** and **[ChartLegendContent](/ui/docs/components/chart#chartlegendcontent)** render tooltips and legends from the same config, with dot, line, and dashed indicators. The Recharts `Tooltip` and `Legend` are re-exported as `ChartTooltip` and `ChartLegend`. Examples cover area, bar, donut, line, pie, and radial charts, plus a gradient-area composition with a pinned tooltip.
...
New dependency: **`recharts`** (Chart).

### AI chat primitives

**`apps/ui`** adds eight components for composing AI chat interfaces, organized under a new **ai** category: **[Conversation](/ui/docs/components/conversation)** is a `role="log"` thread with stick-to-bottom auto-scroll during streaming, a jump-to-latest button, an empty state, and Markdown download. **[Message](/ui/docs/components/message)** provides role-aware bubbles (`from="user" | "assistant" | "system" | "tool"`), copy/feedback actions, and a branching compound for alternative replies. **[Prompt Input](/ui/docs/components/prompt-input)** is a composer built on `InputGroup` with an auto-grow textarea, Enter-to-submit, and submit-status icons. **[Reasoning](/ui/docs/components/reasoning)** wraps model thinking in a collapsible that auto-opens while streaming, measures the duration, and auto-closes once after a short delay. **[Tool](/ui/docs/components/tool)** renders tool calls with a seven-state status badge, JSON input, and output/error sections. **[Sources](/ui/docs/components/sources)** lists citations, **[Suggestion](/ui/docs/components/suggestion)** renders prompt chips, and **[Shimmer](/ui/docs/components/shimmer)** animates streaming text via the Web Animations API (no global keyframes, disabled under reduced motion).

The components are presentational and AI-SDK-agnostic: role and status unions are declared locally, and message content is plain children — bring your own Markdown renderer. The set composes into a full chat demo in the Conversation examples.

Install them with the shadcn CLI, for example:
...
New dependency: **`use-stick-to-bottom`** (Conversation).

## August 31, 2026

### Nine new primitives

**`apps/ui`** adds nine components: **[Menubar](/ui/docs/components/menubar)** and **[Navigation Menu](/ui/docs/components/navigation-menu)** wrap the Base UI 1.6 primitives of the same name. **[Stepper](/ui/docs/components/stepper)**, **[Timeline](/ui/docs/components/timeline)**, **[Tree](/ui/docs/components/tree)**, **[Resizable](/ui/docs/components/resizable)**, **[Carousel](/ui/docs/components/carousel)**, **[Data Table](/ui/docs/components/data-table)**, and **[Select Native](/ui/docs/components/select-native)** cover the remaining gaps against the Origin UI collection. **`Resizable`** panels support optional snapping: `snapPoints` (plus an `id`) makes a panel settle onto preset sizes — pointer releases snap unconditionally to the nearest point; keyboard resizes snap on zone entry so nudging off a point is never trapped.
...
New dependencies: **`@tanstack/react-table`** (Data Table), **`react-resizable-panels`** (Resizable), **`embla-carousel-react`** (Carousel), and **`@headless-tree/core`** / **`@headless-tree/react`** (Tree). Each component's installation section lists the dependencies it needs.
